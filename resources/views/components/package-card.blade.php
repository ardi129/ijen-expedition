...
<div class="card flex flex-col h-full group">
    <div class="relative overflow-hidden aspect-[4/3] sm:aspect-video">
        ...
        <div class="absolute top-3 sm:top-4 left-3 sm:left-4 flex gap-1.5 sm:gap-2 flex-wrap">
            <span class="inline-flex items-center px-2 sm:px-2.5 py-0.5 rounded-full text-[10px] sm:text-xs font-medium bg-primary-100 text-primary-800 shadow-sm">
                {{ $package->category->name }}
            </span>
            @if($package->is_featured)
            <span class="inline-flex items-center px-2 sm:px-2.5 py-0.5 rounded-full text-[10px] sm:text-xs font-medium bg-accent-100 text-accent-800 shadow-sm">
                Populer
            </span>
            @endif
        </div>

        <div class="absolute bottom-0 inset-x-0 h-1/2 bg-gradient-to-t from-gray-900/80 to-transparent"></div>
    ...
    <div class="p-4 sm:p-6 flex flex-col flex-grow">
        <h3 class="text-base sm:text-xl font-heading font-bold text-gray-900 dark:text-white mb-1.5 sm:mb-2 line-clamp-2 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
            <a href="{{ route('packages.show', $package) }}">{{ $package->title }}</a>
        </h3>

        <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-3 sm:mb-4 line-clamp-2">
        ...
        <div class="mt-auto mb-3 sm:mb-5 flex flex-wrap gap-1 sm:gap-1.5">
            @foreach($package->destinations->take(3) as $dest)
                <span class="inline-flex items-center text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">
                    <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5 mr-0.5 sm:mr-1 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                    {{ $dest->name }}
                </span>
            @endforeach
            @if($package->destinations->count() > 3)
                <span class="text-[10px] sm:text-xs text-gray-400">+{{ $package->destinations->count() - 3 }} lagi</span>
            @endif
        </div>

        <div class="flex items-end justify-between mt-auto pt-3 sm:pt-4 border-t border-gray-100 dark:border-gray-700">
            <div>
                @if($package->price > 0)
            ...
            <a href="{{ route('packages.show', $package) }}" class="inline-flex items-center justify-center p-1.5 sm:p-2 rounded-full bg-primary-50 text-primary-900 hover:bg-primary-900 hover:text-white transition-colors dark:bg-gray-700 dark:text-primary-300 dark:hover:bg-primary-800 dark:hover:text-white" title="Lihat Detail">
